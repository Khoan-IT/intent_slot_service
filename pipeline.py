...
class IntentSlotModel:
    ...
    def _predict(self, input_data: Dict[str, torch.Tensor], words: List[str])-> str:
        ...
        result = {
            "intent": self.intent_label_lst[intent_preds[0]],
            "slot": slots
        }
    ...
    def __call__(self, input_message: str) -> str:
        words = input_message.strip().split()
        return self._predict(self._convert_input_to_tensor(words), words)
...
if __name__ == "__main__":
    ...
    for _ in range(1): 
        sentence = 'cảm ơn admin rất nhiều'
